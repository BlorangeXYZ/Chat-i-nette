...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  wss.on('connection', (ws) => {
    ...
    const chainPromise = WeaviateStore.fromExistingIndex(new OpenAIEmbeddings(), { client, indexName: SCHEMA_NAME, textKey: 'text' }).then((vs) => makeChain(vs, onNewToken));
    ...
    ws.on('message', async (data) => {
      try {
        ...
        //const resultTwo = chain.stream()
        const result = await chain.call({
            question,
            chat_history: chatHistory,
        });
        console.log(`Result LLM END: ${result.answer}`);
